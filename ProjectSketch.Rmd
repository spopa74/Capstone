...
**DECISIONS**  
There is a serious amount of data, which makes working with the whole set of "high-n"-grams impractical. So, first thing, I'll truncate the number of N-grams to 1 million (keep ALL the 1-grams, then the most frequent 1 million 2-, 3-, 4-, 5-grams).**  
...
* truncate to 1 million  
* extract the "N-gram to index" vector, save  
* extract the priors (N-1-Grams), posteriors (1-Grams), save
* calculate the probabilities for the N-Grams, using the formula   
  * P(w1w2..wn) = P(wn|w1w2..wn-1)*P(w1w2..wn-1) = ( C(w1w2...wn)/C(w1w2..wn-1) ) * P(w1w2..wn-1)  
* sort and save the probabilities
...
save(indexes5GramNO, file="5_models/indexes5GramNO.tf") ## MAPPING N-GRAM TO INDEX

## find prior, posterior
allNames5 <- names(tf5SortedNO)
freq5GramNO <- unname(tf5SortedNO)
save(freq5GramNO, file="5_models/freq5GramNO.tf")       ## MAPPING INDEX TO COUNTS
...
## it's possible that some prior for an N-gram is not in the list of the N-1 grams
## in that case, use the lowest possible value for the N-1 probability, calculate 
## in a previous step). 
...
save(prior5Ix, file="5_models/prior5GramNO.tf")     ## MAPPING INDEX TO PRIOR
save(post5Ix, file="5_models/posterior5GramNO.tf")  ## MAPPING INDEX TO POSTERIOR
...
save(prob5GramNO, file="5_models/prob5GramSimple.tf") ## MAPPING N-GRAM TO PROBABILITY
...
**DECISIONS**
Not going to use perplexity measurements. While in general, would give a somewhat accurate measure of "how probable is a certain sentence" (using 5-Grams in my model), would not measure IF the keyboard would be actually useful. Our interest here is to give the user "good options", not necessarily a "high probability of sentence". It's very possible that a model with a lower perplexity would give NO useful predictions to the user. 
Otherwise, should be trivial to calculate:  
* loop through the test text.   
* for each word calculate the probability of that word in the context (5-Grams if we have 4 previous words, etc). 
* log the probability, add it and then calculate the mean - this would be the perplexity.   
    
So, with the accuracy measurements described above, we arrive to the following results:  
